#include "SignalizedIntersection.h"
...
void IntersectionCarFollowing_old(car_data& curr_data, car_data& prev_data, bool method=false) {
	//OutputDebugString("Your debug message here CF Signal\n");
//=========================================================
	// PARAMETERS

	// Car following parameters
	double dx_min = 2; // m  [Lin] This can be changed to lower value for a shorter standstill distance 
	double desired_timegap = 1.6; // s
	double a_max_0 = 3.5; // m/s2
	double a_max_80 = 3; // m/s2
	double a_min = -4; // m/s2
	...
	double dx_current = curr_data.veh_distance_headway;
	double dv_current = curr_data.lead_veh_rel_velocity;

	// Delayed sensor values
	//DMFunctions::DelayDataBase(curr_data, prev_data, 5);
	//double dx_current = curr_data.veh_past_distance_headway_5 - dx_min;
	//double dv_current = curr_data.veh_past_rel_vel_5;
	...
	if (curr_data.veh_active_gap_searching) {
		//std::string message = "Active Gap Search when Approacing Signa by Vehicle ID: " + std::to_string(curr_data.veh_id) + "\n";
		//OutputDebugString(message.c_str());
		speed_limit = curr_data.veh_BADF_temporary_setspeed;
	}

	// Specifying values for RED Light -- ADDED BY SAEED 
	if (method) {
		thw_current = curr_data.veh_signal_distance / v_current;
		dv_current = curr_data.veh_velocity;
		dx_current = curr_data.veh_signal_distance; //[Lin] removed dx_min to avoid double counting
		v_front_veh = 0;
		v_front_veh_pre = 0;
		// speed_limit = 0;
	}
	...
	double a_max = (a_max_80 * (80 - v_current * 3.6) + a_max_0 * (v_current * 3.6 - 0)) / 80;
	if (a_max > a_max_80) { a_max = a_max_80; };
	if (a_max < a_max_0) { a_max = a_max_0; };
	...
	*/
	...
	// distance error
	double dv_des = CF_params.K_d * (desired_timegap * v_current - dx_current + dx_min);
	...
	// OLD IMPLEMENTATION (without considering intersections)
	//double a_des = a_des_free;
	//if (curr_data.lead_veh_type != 0) {
	//	a_des = min(a_des_cf, a_des_free);
	//} 

	// NEW IMPLEMENTATION (considering intersections)
	double a_des = a_des_free;
	if (method) { 
		a_des = min(a_des_cf, a_des_free); 
	}
	...
	*/
	...
	curr_data.veh_BADF_desired_aceleration_signalized_intersection = a_des;
	
	//=========== code for breakpoint =========================
	/*if (curr_data.veh_id == 2 && curr_data.timestep >= 15) {
		int bug = 0;
	}*/
	//=========================================================
...
void IntersectionCarFollowing(car_data& curr_data, car_data& prev_data, bool method = false) {
...
//=========================================================
	...
	// Delayed sensor values
	DMFunctions::DelayDataBase(curr_data, prev_data, senserDelay); 
	double dx_current = curr_data.veh_used_distance_headway; 
	...
	*/
	...
	// Milans, Vicente / Shladover, Steven E. Handling Cut - In Vehicles in Strings of Cooperative Adaptive Cruise Control Vehicles. Journal of Intelligent Transportation Systems.
	...
	else  //[Lin] need to add else condition, otherwise the temprary time gap will be calculated twice at first swithcing moment.
	{
	...
	*/
	...
	curr_data.veh_BADF_desired_aceleration_signalized_intersection = a_final;

	// curr_data.veh_BADF_desired_aceleration_signalized_intersection = a_des; old return
...
/// <summary>
/// From now on, this is the control logic for specifying the desired acceleration at the signalized intersection
/// </summary>
/// <param name="curr_data"></parameters from VISSIM>
/// <param name="prev_data"></parameters from VISSIM>
/// 
void ApproachingIntersection::SignalizedIntersection(car_data& curr_data, car_data& prev_data) {
	
	if (curr_data.veh_id == 6 && curr_data.timestep >= 78) { // 84.8, 91.9, 105.8
		int debug = 1;
	}
	//Specifying the method for the control logic at signalized intersection
	bool method = 1; // 0: specifying the deceleration mannually, 1: specifying th deceleration by car-following logic

	// Setting the parameters
	double max_comf_dec = -2.5; // Max Comfortable Deceleration
	double comf_dec = -1.0; // Comfortable Deceleration
	double SignalReactionDistance = 70; // the distance that BADF start to react to a signal head

	if (curr_data.veh_signal_distance >= 0 && curr_data.veh_signal_distance <= SignalReactionDistance) {
		//OutputDebugString("Your debug message here Approaching Signal\n");
		// NOTE: the value of required_acc is negative
		double required_acc = -(curr_data.veh_velocity * curr_data.veh_velocity) / (2 * curr_data.veh_signal_distance);

		// Specifying the deceleration mannually
		if (!method) {
			if (curr_data.veh_signal_state == 3) { // GREEN Light
				IntersectionCarFollowing(curr_data, prev_data);
			}

			else if (curr_data.veh_signal_state == 2) { // YELLOW Light

				if (curr_data.veh_velocity > 0) {
					if (required_acc < max_comf_dec) {
						IntersectionCarFollowing(curr_data, prev_data);
					}
					else {
						curr_data.veh_BADF_desired_aceleration_signalized_intersection = required_acc;
					}
				}
				else {
					curr_data.veh_BADF_desired_aceleration_signalized_intersection = 0;
				}
			}

			else if (curr_data.veh_signal_state == 1) { // RED Light

				if (required_acc <= comf_dec && curr_data.veh_velocity > 0) {
					curr_data.veh_BADF_desired_aceleration_signalized_intersection = required_acc;
				}
				else if (curr_data.veh_velocity == 0 && curr_data.veh_signal_distance < 4) {
					curr_data.veh_BADF_desired_aceleration_signalized_intersection = 0;
				}
				else if (curr_data.veh_velocity == 0 && curr_data.veh_signal_distance >= 1) { //possible bug: the speed of the vehicle does not remain exactly zero at stop or the distance is wrong
					IntersectionCarFollowing(curr_data, prev_data);
				}
				else { // still far from the intersection
					IntersectionCarFollowing(curr_data, prev_data);
				}
			}
		}

		// Specifying the deceleration by car-following logic
		else
		{
			if (curr_data.veh_signal_state == 3) { // GREEN Light
				IntersectionCarFollowing(curr_data, prev_data);
				...
				if (curr_data.veh_velocity > 0) {
					if (required_acc < max_comf_dec) {
						...
						IntersectionCarFollowing(curr_data, prev_data, method = true); // it didn't decelerate hard enough
						// curr_data.veh_BADF_desired_aceleration_signalized_intersection = required_acc;
					}
				}
				else {
				...
				// This Method does not consider our agreements on max and comfortable deceleration
				// I think we can use the next lines when the BADF is able to react to standstill vehicles with hard deceleration
				IntersectionCarFollowing(curr_data, prev_data, method = true);

				// This methods considers our agreements on max and comfortable deceleration
				//if (required_acc <= comf_dec && curr_data.veh_velocity > 0) {
				//	IntersectionCarFollowing(curr_data, prev_data, method = true);
				//}
				//else if (curr_data.veh_velocity == 0 && curr_data.veh_signal_distance < 1) {
				//	curr_data.veh_BADF_desired_aceleration_signalized_intersection = 0;
				//}
				//else if (curr_data.veh_velocity == 0 && curr_data.veh_signal_distance >= 1) { //possible bug: the speed of the vehicle does not remain exactly zero at stop or the distance is wrong
				//	IntersectionCarFollowing(curr_data, prev_data);
		...
		// OUTPUT: curr_data.veh_BADF_desired_aceleration_signalized_intersection = ;
};
